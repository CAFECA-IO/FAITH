...
const ChatBriefItem = ({ chatBrief }: { chatBrief: IChatBrief }) => {
  ...
  const chatBriefClickHandler = (e: React.MouseEvent) => {
    // 防止點擊事件影響拖動
...
const ChatSidebar = ({ getIsExpanded }: ChatSidebarProps) => {
  ...
  const { chatBriefs, folders, addFolder, updateChatBriefs } = useChatCtx();
  ...
  if (!isMounted) {
    return <div>Loading...</div>; // 或者返回一個骨架屏
  }
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    if (result.destination.droppableId === 'add-new-folder') {
      const draggedChatId = result.draggableId;
      const draggedChat = chatBriefs?.find((chat) => chat.id === draggedChatId);

      if (draggedChat) {
        const newFolder: IFolder = {
          id: uuidv4(),
          name: `New Folder ${folders?.length ? folders.length + 1 : 1}`,
          chats: [draggedChat],
        };
        addFolder(newFolder);

        const updatedChatBriefs = chatBriefs?.map((chat) =>
          chat.id === draggedChatId ? { ...chat, folders: [...chat.folders, newFolder.id] } : chat
        );
        if (updatedChatBriefs) {
          updateChatBriefs(updatedChatBriefs);
        }
      }
    }
  };
  ...
  const displayedChatBriefs = signedIn && chatBriefs && chatBriefs.length > 0 && (
    <Droppable droppableId="chat-briefs">
      {(provided) => (
  ...
  console.log('displayedChatBriefs', displayedChatBriefs);
    ...
    >
      <div className="font-barlow">
        ...
        >
          ...
          <Droppable droppableId="add-new-folder">
            {(provided) => (
